...
\tdoclatexin{xvals = mavar : x_1 , x_2 , ... , x_n}
ou juste
\tdoclatexin{xvals = x_1 , x_2 , ... , x_n}
si la variable par défaut, à savoir $x$, convient.
Une fois ceci fait, il faut renseigner les différentes images via \tdoclatexin{imgs = monexpr : im_1 , im_2 , ... , im_n} en donnant obligatoirement la formule de l'expression étudiée.
...
\begin{tdocwarn}
	L'utilisation de \tdoclatexin{xvals} doit se faire obligatoirement une fois, et une seule, au tout début du contenu.
\end{tdocwarn}
...
\begin{tdocexa}[Avec des étiquettes de partout]
    En plus de \tdoclatexin{xvals} pour les colonnes, il faut utiliser \tdoclatexin{yvals}, avec un fonctionnement similaire, pour renseigner les valeurs initiales qui seront utilisées pour chaque ligne.
    Quand aux valeurs de chaque cellule, on les donne sous forme matricielle de façon naturelle via \tdoclatexin{mat} : pour une ligne, les valeurs sont séparées par des virgules, tandis que les points-virgules permettent de passer à la ligne suivante.
...
Tout comme pour les tableaux de signes, via l'option \tdoclatexin{param = true}, un tableau de variations contiendra dans sa dernière ligne les valeurs pivots. Voici un exemple d'utilisation de cette option.
...
%
\begin{enumerate}
    \item Les commentaires se font uniquement à la sauce \LaTeX\ via \tdoclatexin{% Mon commentaire} par exemple.
    ...
    \begin{itemize}
        ...
        \item Un nouveau contexte s'indique en début de contenu via \tdoclatexin{nom_contexte = contenu_1 ; ...} sans nécessité d'indiquer à chaque fois le contexte.
...
%
\begin{enumerate}
    \item Le contexte \tdoclatexin{xvals} doit être employé une seule fois en tout début de code, hors commentaires, c'est-à-dire avant le contexte \tdoclatexin{imgs}.


    \item Un seul contenu est utilisable en utilisant l'un des deux formats suivants où le nombre de valeurs n'est pas majoré.
    %
    \begin{itemize}
        \item \tdoclatexin{ma_{var} : x_1 , x_2 , ... , x_n} indique que les $n$ valeurs \tdoclatexin{x_1} , \tdoclatexin{x_2} , \dots\ , \tdoclatexin{x_n} ont été données pour la variable nommée $ma_{var}$ avec $n \geq 2$ forcément.

        \item \tdoclatexin{x_1 , x_2 , ... , x_n} indique que $n$ valeurs pivots \tdoclatexin{x_1} , \tdoclatexin{x_2} , \dots\ , \tdoclatexin{x_n} ont été données pour la variable nommée $x$ qui est le nom par défaut.
    \end{itemize}
\end{enumerate}
...
\subsubsection{Le contexte \tdoclatexin{imgs} pour les images}

Le contexte \tdoclatexin{imgs} s'utilise de façon similaire au contexte \tdoclatexin{xvals} mais avec la possibilité de l'employer autant de fois que nécessaire.
Par contre, il est obligatoire d'indiquer l'expression  via
\tdoclatexin{mon_expr : img_1 ,}
\tdoclatexin{img_2 , ... , img_n}
où chaque \tdoclatexin{img_k} peut être soit \tdocquote{vide}\,, soit une expression mathématique \LaTeX\,.
...
\begin{enumerate}
    ...
    \item L'expression dont on indique le signe doit être donnée via \tdoclatexin{mon_expr : ...} où les points de suspension spécifient les informations liées au comportement de l'expression (voir l'item suivant à ce sujet).
    \textbf{Il n'existe pas d'expression par défaut.}


    \item Dans cet item, pour expliquer ce qui est attendu comme informations relatives au comportement d'une expression $f$ , nous allons supposer que $n$ valeurs pivots \tdoclatexin{x_1} , \tdoclatexin{x_2} , \dots\ , \tdoclatexin{x_n} ont été données via \tdoclatexin{xvals = x_1 , x_2 , ... , x_n} . Nous posons aussi $p = n - 1$ avec \tdocpre{p} pour \tdocquote{\tdocprewhy{p.récédent} le naturel $n$} .
    %
    \begin{itemize}
        \item A minima, il faut indiquer
        \tdoclatexin{f : s_1 s_2 ... s_p}
        où
        \tdoclatexin{s_1} , \tdoclatexin{s_2} , \dots\ , \tdoclatexin{s_p}
        donnent des informations sur les $p$ intervalles
        \tdoclatexin{]x_1 ; x_2[} , \tdoclatexin{]x_2 ; x_3[} , \dots\ , \tdoclatexin{]x_p ; x_n[} respectivement.
        Les valeurs possibles pour les \tdoclatexin{s_k} sont les suivantes.
        %
        \begin{enumerate}
        ...
        %
        \begin{itemize}
            \item Si $n > 3$ alors $f(x_3) = 0$ s'indique via \tdoclatexin{... 0 s_3 ...} au milieu du code.

            \item Si $n = 3$ alors $f(x_3) = 0$ s'indique via \tdoclatexin{... s_2 0} en fin de code.

            \item $f(x_1) = 0$ s'indique via \tdoclatexin{0 s_1 ...}    au début du code.
        \end{itemize}
        %
...
%
\begin{enumerate}
    \item Voici les informations possibles pour le comportement d'une expression $f$ en supposant que $n$ valeurs pivots \tdoclatexin{x_1} , \tdoclatexin{x_2} , \dots\ , \tdoclatexin{x_n} ont été données via \tdoclatexin{xvals = x_1 , x_2 , ... , x_n} . De nouveau, nous posons $p = n - 1$ .
    %
    \begin{itemize}
        \item A minima, il faut indiquer
        \tdoclatexin{f : v_1 v_2 ... v_p}
        où
        \tdoclatexin{v_1} , \tdoclatexin{v_2} , \dots\ , \tdoclatexin{v_p}
        donnent des informations sur les $p$ intervalles ouverts
        \tdoclatexin{]x_1 ; x_2[} , \tdoclatexin{]x_2 ; x_3[} , \dots\ , \tdoclatexin{]x_p ; x_n[} respectivement.
        Les valeurs possibles pour les \tdoclatexin{v_k} sont les suivantes.
        %
        \begin{enumerate}
...
Se reporter à la section \ref{tns-math-functab-dsl-l3-ctxt-xvals} donnant la syntaxe attendue, mais avec une petite différence : le contexte \tdoclatexin{xvals} peut être précédé du contexte \tdoclatexin{yvals} présenté dans la section suivante.
...
La logique est la même que pour le contexte \tdoclatexin{xvals}, de nouveau en ne pouvant placer que le contexte \tdoclatexin{xvals} avant le contexte \tdoclatexin{yvals}.
...
Le contexte \tdoclatexin{mat} qui n'est utilisable qu'une seule fois, permet de donner la matrice des données, et ceci en indiquant, si besoin uniquement
\footnote{
