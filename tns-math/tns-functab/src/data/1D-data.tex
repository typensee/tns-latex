...
%
\begin{enumerate}
	\item \tdocinlatex{xvals = x_1 , x_2 , ... , x_n} indiquent les valeurs pour la variable nommée $x$ par défaut.

	\item \tdocinlatex{xvals = mavar : x_1 , x_2 , ... , x_n} permet de renommer la variable en $mavar$\,.
\end{enumerate}


Une fois ceci fait, il faut renseigner les différentes images en donnant obligatoirement la formule de l'expression étudiée en passant via \tdocinlatex{imgs = monexpr : im_1 , im_2 , ... , im_n}\,.

\medskip

En résumé, tout se passe via \tdocinlatex{xvals = ...} et \tdocinlatex{imgs = ...} pour définir les valeurs pivots et leurs images.
...
\begin{tdocwarn}
	L'utilisation de \tdocinlatex{xvals} doit être faite une fois, et une seule, au tout début du contenu.
\end{tdocwarn}
