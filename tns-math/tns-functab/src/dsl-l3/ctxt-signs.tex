...
\begin{enumerate}
    ...
    \item L'expression dont on indique le signe doit être donnée via \tdocinlatex{monexpr : ...} où les points de suspension spécifient les informations liées au comportement de l'expression (voir l'item suivant à ce sujet).
    ...
    \begin{itemize}
        ...
        %
        \begin{itemize}
            \item Si $n > 3$ alors $f(x3) = 0$ s'indique via \tdocinlatex{... 0 s3 ...} au milieu du code.

            \item Si $n = 3$ alors $f(x3) = 0$ s'indique via \tdocinlatex{... s2 0} en fin de code.

            \item $f(x1) = 0$ s'indique via \tdocinlatex{0 s1 ...}    au début du code.
        \end{itemize}
        %
