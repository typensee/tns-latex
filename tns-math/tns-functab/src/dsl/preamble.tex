\documentclass[12pt, a4paper]{article}

\input{../preamble.cfg.tex}

%\usepackage{../macroenv/macroenv}


\begin{document}

\section{La syntaxe du langage proposé}

Comme ceci a été montré dans les sections précédentes, \tdocpack{tns-functab} facilite la saisie de différents types de tableaux liés à des fonctions en proposant un langage de saisie intuitif
\footnote{
	En informatique, on parle de \texttt{DSL} soit \tdocquote{Domain Specific Language}\,, c'est-à-dire  \tdocinEN{Langue spécifique à un domaine}\,.
}. Cette section explique simplement les règles syntaxiques et sémantiques de ce langage.

\end{document}
